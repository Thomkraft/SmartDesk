<script>
  ...
  function formatNumber(num) {
    ...
    if (!Number.isFinite(num) || Math.abs(num) > MAX_VALUE)
      return "Err: Dépassement";
  ...
  function handleNumber(num) {
    ...
    display = formatDisplayNumber(
      display === "0" ? num.toString() : display.replace(/\s/g, "") + num
    );
  }
...
>
  ...
  <div class="grid grid-cols-4 gap-2 p-4 flex-grow">
    ...
    <button on:click={() => handleOperation("÷")} class="btn bg-orange-500"
      >÷</button
    >
    ...
    {#each [[7, 8, 9, "×"], [4, 5, 6, "-"], [1, 2, 3, "+"]] as row}
      {#each row as btn}
        <button
          on:click={() =>
            typeof btn === "number" ? handleNumber(btn) : handleOperation(btn)}
          class="btn {typeof btn === 'number'
            ? 'bg-gray-600'
            : 'bg-orange-500'}"
        >
          {btn}
    ...
    <button on:click={() => handleNumber(0)} class="btn bg-gray-600 col-span-2"
      >0</button
    >
